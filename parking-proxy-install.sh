...
func (f *MPGSFetcher) Fetch(ctx context.Context) ([]SpaceInfo, error) {
    ...
    // Логирование запроса и ответа MPGS
    log.Info().
        Str("url", f.apiURL).
...
func (p *Processor) Process(data []SpaceInfo, cfg *Config, isDay bool) (map[string]TablePayload, bool) {
    ...
    // Строим мапы: zone -> free, floor -> free
    ...
    // Проверяем изменения по количеству свободных мест для каждой row
    changed := false
...
func buildIndexHTML(cfg *Config) string {
    ...
    for i, t := range cfg.Tables {
        ...
        for j := 1; j <= 4; j++ {
            ...
            <div class="row-data" id="table_%d_row%d_container">
                ...
                <div style="margin-top:5px">
                    ...
                    <span style="margin-left:10px"><strong>Floors:</strong> <span id="table_%d_row%d_floors_display">-</span></span>
                </div>
                <div class="checkbox-group">
                    <div id="table_%d_row%d_zonecheckboxes" style="display:inline">Zones: no data</div>
                    <div id="table_%d_row%d_floorcheckboxes" style="display:inline;margin-left:10px">Floors: no data</div>
                </div>
        ...
        <div class="table-block" id="table_%d">
            ...
            <div class="form-group"><label>is_day</label><select id="table_%d_isday"><option value="true">true (day)</option><option value="false">false (night)</option></select></div>
...
<body>
    ...
    <script>
        ...
        function updateRowCheckboxes(tableIdx, rowIdx, data) {
            ...
            if (!data) {
                zoneDiv.innerHTML = 'Zones: no data';
                floorDiv.innerHTML = 'Floors: no data';
                return;
            }
        ...
        function updateRowValue(tableIdx, rowIdx) {
            ...
            if (zonesDisplay) zonesDisplay.textContent = selectedZones.join(', ') || 'any';
            if (floorsDisplay) floorsDisplay.textContent = selectedFloors.join(', ') || 'any';
        }
        ...
        function addTable() {
            ...
            for (var j = 1; j <= 4; j++) {
                rowsHTML += '<div class="row-data" id="table_' + i + '_row' + j + '_container">' +
                    ...
                    'Text: <input id="table_' + i + '_row' + j + '_text" value="" style="width:80px" placeholder="0" type="number"> ' +
                    'Img: ' + buildImgSelectHTML('table_' + i + '_row' + j + '_img', imgOptions[j-1]) +
                    '<div style="margin-top:5px">' +
            ...
            div.innerHTML = '<button class="remove-btn danger" onclick="removeTable(' + i + ')">X</button>' +
                ...
                '<div class="col"><label>Port</label><input type="number" id="table_' + i + '_port" value="8090"></div></div>' +
                '<div class="form-group"><label>is_day</label><select id="table_' + i + '_isday"><option value="true">true (day)</option><option value="false">false (night)</option></select></div>' +
